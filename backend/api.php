...
try {
    switch ($action) {
        ...
        case 'report':
            ...
            $start = requireDateYmd(substr($start, 0, 10), 'start');
            $endYmd = requireDateYmd(substr($end, 0, 10), 'end');
            // keep previous semantics for daily_sales filtering
            $end = $endYmd . ' 23:59:59';
            
            $daily = $supabase->select('daily_sales', "?date=gte.$start&date=lte.$end&order=date.asc");
            $summary = $supabase->request('POST', 'rpc/get_product_summary', ['start_date' => $start, 'end_date' => $end]);
            
            echo json_encode([
                'daily' => json_decode($daily['body']),
                'summary' => json_decode($summary['body'])
            ]);
            break;
